...
for language_pair in "en-yor" "en-hau" "en-tsn" "fr-fon" "fr-wof" "en-swa"
do
    IFS='-' read -ra ADDR <<< "$language_pair"
    for SEED in {1..3}
    do
        for augmentation_percent in 20 30
        do
            echo "Running $language_pair, seed number $SEED and $augmentation_percent"
            CUDA_VISIBLE_DEVICES=1 python \
                ../src/finetining/mbart/hf_seq2seq_translation.py \
                --model_name_or_path facebook/mbart-large-50 \
                --do_train \
                --do_eval \
                --do_predict \
                --source_lang  ${ADDR[0]}_XX \
                --target_lang xh_ZA \
