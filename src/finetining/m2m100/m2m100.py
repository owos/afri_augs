...
def main():
    ...
    # For translation we set the codes of our source and target languages (only
    # useful for mBART, the others will ignore those attributes).
    if isinstance(tokenizer, tuple(MULTILINGUAL_TOKENIZERS)):
        assert (
            data_args.target_lang is not None and data_args.source_lang is not None
        ), (
            f"{tokenizer.__class__.__name__} is a multilingual tokenizer which "
            "requires --source_lang and  --target_lang arguments."
        )

        tokenizer.src_lang = data_args.source_lang
        tokenizer.tgt_lang = data_args.target_lang

        # For multilingual translation models like mBART-50 and M2M100 we need to
        # force the target language token as the first generated token. We ask
        # the user to explicitly provide this as --forced_bos_token argument.
        forced_bos_token_id = (
    ...
    if training_args.label_smoothing_factor > 0 and not hasattr(
        ...
        logger.warning(
            "label_smoothing is enabled but the `prepare_decoder_input_ids_from"
            "_labels` method is not defined for `%s`. "
