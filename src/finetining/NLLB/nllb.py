...
class ModelArguments:
    ...
    """arguments for the model/tokenizer"""

    model_name_or_path: str = field(
        metadata={
            "help": "path to the pretrained model or model identifier from huggingface"
        }
    )
    ...
    config_name: Optional[str] = field(
        default = None,
        metadata={
            "help": "Pretrained config name or path if not same as model_name"
        }
    )
    ...
    cache_dir: Optional[str] = field(
        default=None,
        metadata={
            "help": "where to store the pretrained models downloaded from hugginface"
        },
    )
...
def main():
  ...
  # We resize the embeddings only when necessary to avoid index errors. If you are creating a model from scratch
    ...
    if model.config.decoder_start_token_id is None and isinstance(
        tokenizer, (NllbTokenizer, NllbTokenizerFast)
    ):
